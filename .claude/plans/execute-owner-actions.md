...
**UPDATE (2026-08-17): 5b fully closed (digestabot/octo-sts); 5e REJECTED — `id-token: write` is load-bearing.** The digestabot half of 5b failed the first time it mattered: the nightly `Update image digests` runs on 08-16 and 08-17 both died on the push with `remote: Permission to mbarretta/brushpass.git denied to github-actions[bot]` (403). Cause was the 08-14 Dependabot bump of `chainguard-dev/digestabot` 1.3.1→1.3.2 (PR #72), which replaced `peter-evans/create-pull-request` with a raw `git push` authenticated through a `credential.helper store` holding the octo-sts token — but `actions/checkout` runs first with `persist-credentials: true`, and the `http.https://github.com/.extraheader` Authorization header it leaves in git config is sent unconditionally, outranking the helper. So git never consulted the octo-sts credential and pushed as github-actions[bot], which this job caps at `contents: read`. 08-14 and 08-15 stayed green only because no digest had drifted, so no push was attempted — the break was latent for two nights. Fixed by PR #97 (`1a945a2b`): `persist-credentials: false` on the checkout step, keeping the read-only credential out of the tree so the App token (`contents: write` per `digestabot.sts.yaml`) is the one git uses. Verified by two `workflow_dispatch` runs on main (32068733250, 32068750612): both pushed the two drifted `node:26-dev`/`node:26-slim` digests, opened digest PR #98, and ran the "Enable auto-merge on digest PR" step that had been `skipped` in every failing run; #98 auto-merged. `claude-review` also passed on #98, confirming the `allowed_bots: 'octo-sts[bot]'` gate live on an octo-sts-authored PR. **5e must not be done as written:** `claude-code-action` v1.0.191 obtains its GitHub token *through* the OIDC exchange — run 32068782907 logs `Requesting OIDC token...` → `Exchanging OIDC token for app token...` → `Using GITHUB_TOKEN from OIDC` — which is precisely why both Claude jobs can hold only read scopes and still post reviews. Dropping `id-token: write` would break the review, not harden it; task 9's "belt-and-braces" premise does not hold for this action version, so 5e is closed as rejected rather than done. Housekeeping: the local branch `fix/claude-review-allow-octo-sts` (`40dc2e1`) is OBE — its `allowed_bots` line is already on main.
...
- 5b. WIF post-verify: `workflow_dispatch` deploy.yml once — proves the pinned `job_workflow_ref` condition still admits the deployer; watch the next scheduled digestabot run for octo-sts. **CLOSED 2026-08-17** — the digestabot watch caught a real break (v1.3.2 push rewrite vs. checkout's persisted token); fixed in PR #97 and verified by dispatch. See the status update above.
...
- 5e. Optional hardening follow-up PR: drop `id-token: write` from the two Claude workflows (task 9's belt-and-braces note). **REJECTED 2026-08-17** — `claude-code-action` mints its write-capable GitHub token via the OIDC exchange, so the permission is load-bearing and removing it breaks the review; that is also why both jobs can stay read-only. See the status update above.
